...
import { IonPage, IonHeader, IonToolbar, IonTitle, IonContent, IonButtons, IonLabel, IonButton, IonIcon, IonGrid, IonRefresher, IonRefresherContent, useIonViewWillEnter } from '@ionic/react';
...
const PageLightDetail : React.FC<ILightDetailProps> = ({match}) => {
  ...
  const update = () => (
    get({url: `/lights/${match.params.id}`}).then(data=>{
      setLight(data)
    })
  )
